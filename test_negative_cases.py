...
def test_post_invalid_data():
    ...
    assert result.json() == {'error': 'Content-Type not supported!'}
    assert result.status_code == 200


def test_post_no_data():
    result = first.post_request()
    assert result.json() == {'error': 'Content-Type not supported!'}  # or we could add another error for this one like "no payload provided"
    assert result.status_code == 200
...
def test_post_no_auth():
    ...
    assert result.json() == {"error": "No authorization credentials provided"}
    assert result.status_code == 200
...
def test_post_wrong_token():
    ...
    payload = {"url": "http://example.com"}
    result = first.post_request(payload)
    assert result.json() == {"error": "wrong token code"}
    assert result.status_code == 200
...
def test_post_wrong_credentials():
    ...
    payload = {"url": "http://example.com"}
    result = first.post_request(payload)
    assert result.json() == {"error": "wrong credentials"}
    assert result.status_code == 200
